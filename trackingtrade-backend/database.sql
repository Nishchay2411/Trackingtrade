...
CREATE TABLE IF NOT EXISTS trades (
  ...
  -- Item 12: every trades query filters by user_id first, often combined
  -- with status/open_time, so these cover the actual query patterns in
  -- tradeController.js and analyticsController.js.
  INDEX idx_trades_user (user_id),
  INDEX idx_trades_user_status (user_id, status),
  INDEX idx_trades_user_opentime (user_id, open_time),
...
-- ── SAMPLE DATA ──
INSERT INTO users (name, email, password, plan, is_verified) VALUES
('John Doe', 'john@example.com', '$2a$10$examplehashedpassword', 'pro', TRUE),
('Sarah Chen', 'sarah@example.com', '$2a$10$examplehashedpassword', 'elite', TRUE);
